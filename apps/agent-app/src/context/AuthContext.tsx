...
// FIX: Provide a safe default so useAuth() outside provider fails gracefully
...
// FIX: Response interceptor for automatic token refresh
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
 ...
 // FIX: Role gate — reject non-agents immediately
 if (userData.role !== 'agent') {
   throw new Error('This portal is for station agents only');
 }
 ...
 if (__DEV__) console.log('[Agent Auth] Token invalid, clearing:', err?.response?.data?.error || err.message);
 localStorage.removeItem('agent_token');
 localStorage.removeItem('agent_refresh_token');
 localStorage.removeItem('agent_user');
 delete api.defaults.headers.common['Authorization'];
 ...
 // FIX: Role gate — only agents allowed
 if (userData.role !== 'agent') {
   return { success: false, error: 'This portal is for station agents only' };
 }
