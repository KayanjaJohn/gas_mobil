...
// HIGH FIX: No sandbox fallback in production — hard fail if env missing
const BASE_URL = process.env.MTN_MOMO_BASE_URL;
if (!BASE_URL) {
  throw new Error("MTN_MOMO_BASE_URL is required in environment");
}
      ...
      {},
      {
        headers: {
          Authorization: `Basic ${Buffer.from(
            `${process.env.MTN_MOMO_DISBURSEMENT_API_USER || API_USER}:${process.env.MTN_MOMO_DISBURSEMENT_API_KEY || API_KEY}`
          ).toString("base64")}`,
          "Ocp-Apim-Subscription-Key": process.env.MTN_MOMO_DISBURSEMENT_SUBSCRIPTION_KEY || SUBSCRIPTION_KEY,
        },
      }
