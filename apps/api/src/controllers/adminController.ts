...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      success: true,
      data: { totalOrders, pendingOrders, totalDrivers, totalProducts, totalStations, todayRevenue }
    });
  } catch (error: any) {
...
export const createStation = async (req: Request, res: Response) => {
  try {
    ...
    const { name, address, latitude, longitude, phone, email, agentName, agentPassword } = req.body;

    if (!email) {
      return res.status(400).json({ success: false, error: 'Station email is required for agent login' });
    }

    // Check if station email already exists as agent login
    const existingAgent = await userRepository.findOne({ where: { email } });
    if (existingAgent) {
      return res.status(400).json({ success: false, error: 'This email is already in use by another station/agent' });
    }
  ...
  try {
    ...
    if (email && email !== station.email) {
      ...
      if (existing && existing.stationId !== id) {
        return res.status(400).json({ success: false, error: 'Email already in use by another station' });
      }
    }
