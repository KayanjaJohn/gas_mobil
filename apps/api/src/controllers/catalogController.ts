...
// GET /api/catalog — List all predefined products (public)
...
// POST /api/catalog — Admin adds a predefined product
...
// PUT /api/catalog/:id — Admin updates catalog item
...
// DELETE /api/catalog/:id — Admin removes catalog item
...
// POST /api/catalog/:id/add-to-station — Admin/Agent adds catalog item to station stock
export const addCatalogToStation = async (req: Request, res: Response) => {
  try {
    ...
    // Check if product already exists at this station
    const existing = await productRepo.findOne({
      where: { name: catalogItem.name, stationId: targetStationId },
    });

    if (existing) {
      return res.status(400).json({
        success: false,
        error: "This product already exists at this station",
      });
    }
